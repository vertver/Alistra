...
public:	
	...
	virtual void NoteOn(u32 Delta, u8 NoteNum, u8 NoteVelocity) 
	{
		f32 PrevSample = CurrentSamples;
		size_t Index = (size_t)((PrevSample * (f32)Samplerate) > DataLen ? DataLen : (PrevSample * (f32)Samplerate));
		size_t FramesToUpdate = 0;

		CurrentTick += Delta;
		CurrentSamples += Delta * CurrentSecondsPerTick;

		FramesToUpdate = (size_t)((CurrentSamples - PrevSample) * (f32)Samplerate);

		if (PrevSample != CurrentSamples)
		{
			f32* DataPart[2] = 
			{
				&Data[0][Index],
			...
			Synthsave.Process(DataPart, FramesToUpdate);
		}

		Synthsave.NoteOn(NoteNum, NoteVelocity);
	}
...
{
private:
	f32* pSynthBuffers[MAX_SYNTHS][2];
	size_t BufSizes[MAX_SYNTHS];
	size_t BufPosition[MAX_SYNTHS];

	CEffectsClass* pEffects[MAX_SYNTHS];
	MidiRender* midiParser[MAX_SYNTHS];
	WAVE_FMT fmt;
...
public:
	...
	void Render(size_t Synth, f32** pBuffer, size_t Frames);
};
